...
do
	...
	sleep 0.2
	    ...
	    # remove dummy atom
	    gro=$( ls $N/pr.gro )
	    #replace number of atoms with dummy atoms in gro File with correct number without dummy atoms 
	    wc_prior=$( wc -l $gro | awk '{print $1'})
            atoms_prior="$(($wc_prior-3))"

	    awk '!/S9/' $gro > temp-dum && mv temp-dum $gro

            wc_posterior=$( wc -l $gro | awk '{print $1'})
            atoms_posterior="$(($wc_posterior-3))"
            
	    sed -i "s/"$atoms_prior"/"$atoms_posterior"/" $gro
...
echo " "
        echo q 
} | gmx make_ndx -f $N/pr.gro  -o   >& out || { cat out; exit 1 ;}
...
molgroup=$( grep "\[" index.ndx | awk ' $2=="'$molname'" {print NR-1 }' )
            ...
            echo q
            } |     gmx make_ndx -f $N/pr.gro -n index.ndx -o index.ndx  >& out || { cat out; exit 1 ;}
            
            
            cat $start_dir/Octasulfur_Octasulfur.ndx index.ndx >> index.ndx
fi # index file 
